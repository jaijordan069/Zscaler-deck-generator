...
st.set_page_config(page_title="Zscaler Transition Deck", layout="wide")

st.markdown("<style>.stApp {background: linear-gradient(to bottom, #0066CC, white);}</style>", unsafe_allow_html=True)
...
st.markdown("Fill → Preview → Generate → Download")

def is_valid_date(s): return bool(re.match(r'^\d{2}/\d{2}/\d{4}$', s))

# === FORM (add your full form code here from previous messages) ===
# ... (milestones, rollout, objectives, deliverables, technical, open items, next steps, contacts) ...
...
if st.button("Generate Deck"):
    if not customer_name:
        ...
        # Slide 1
        s1 = add_title_slide("Professional Services Transition Meeting", f"{customer_name}\n{today_date}")
        ...
        if img.status_code == 200:
            ...
            s1.shapes.add_picture(pic, Inches(0), Inches(0), prs.slide_width, prs.slide_height)
        ...
        ca = zia.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(2), Inches(2), Inches(2), Inches(1))
        ...
        tun = zia.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(5), Inches(2), Inches(2), Inches(1))
        ...
        short = ns.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(1.5), Inches(4.5), Inches(4))
        ...
        long = ns.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(5), Inches(1.5), Inches(4.5), Inches(4))
        ...
        tf.clear()
        p = tf.add_paragraph()
        p.text = f"Your feedback... Project Manager: {pm_name} Consultant: {consultant_name} ... Primary Contact: {primary_contact} Secondary Contact: {secondary_contact} ..."
        for para in tf.paragraphs:
            para.font.size = Pt(14)
            para.font.color.rgb = RGBColor(0,0,0)
        bubble = th.shapes.add_shape(MSO_SHAPE.CLOUD_CALL_OUT, Inches(7), Inches(3), Inches(2), Inches(1))
