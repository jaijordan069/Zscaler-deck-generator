...
def set_font_run(run, name=FONT_NAME, size=SIZE_BODY, bold=False, color=COLOR_BLACK):
    ...
    This works across python-pptx versions by using the public run.font API and
    attempting safe fallbacks to set East-Asian font (rFonts) if available.
    """
    # Public API (safe)
    run.font.name = name
    run.font.size = size
    ...
    # Try to set East Asian font (rFonts) for better font fidelity when supported.
    # Different python-pptx versions expose underlying XML differently; try both safe variants.
    try:
        ...
        try:
            ...
            # Not critical — continue without setting rFonts
            pass
...
def add_textbox(slide, left, top, width, height, text, size=SIZE_BODY, bold=False, color=COLOR_BLACK, align=PP_ALIGN.LEFT, auto_size=False):
    ...
    p.alignment = align
    run = p.runs[0]
    set_font_run(run, name=FONT_NAME, size=size, bold=bold, color=color)
    return txBox
...
if st.button("Generate Transition Deck"):
    ...
    def create_zia_diagram_slide(slide_num=1):
        ...
        for idx, line in enumerate(lines):
            p = kbf.add_paragraph()
            p.text = line
            set_font_run(p.runs[0], size=SIZE_BODY if line else SIZE_BODY, bold=False, color=COLOR_BLACK)
            p.alignment = PP_ALIGN.LEFT
